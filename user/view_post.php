...
<section class="view-post">
    ...
    <?php
    $select_post = $connForPosts->prepare("SELECT * FROM `rating_posts` WHERE id = ? LIMIT 1");
    ...
    if ($select_post->rowCount() > 0) {
        while ($fetch_post = $select_post->fetch(PDO::FETCH_ASSOC)) {
            ...
            $select_ratings = $connForPosts->prepare("SELECT * FROM `rating_reviews` WHERE post_id = ?");
            ...
            <div class="row">
                <div class="col">
                    <img src="../uploaded_files/post_images/<?= $fetch_post['image']; ?>" alt="" class="image">
...
<section class="reviews-container">
    ...
    <div class="box-container">
        <?php
        $select_reviews = $connForPosts->prepare("SELECT * FROM `rating_reviews` WHERE post_id = ?");
        ...
        if ($select_reviews->rowCount() > 0) {
            while ($fetch_review = $select_reviews->fetch(PDO::FETCH_ASSOC)) {
                ...
                <div class="box" <?php if ($fetch_review['user_id'] == $user_id) { echo 'style="order: -1;"'; }; ?>>
                    <?php
                    $select_user = $connForAccounts->prepare("SELECT * FROM `rating_accounts` WHERE id = ?");
                    ...
                    <?php if ($fetch_review['user_id'] == $user_id) { ?>
                        <form action="" method="post" class="flex-btn">
                            ...
                            <input type="button" value="Delete Review" class="inline-delete-btn" id="delete-btn-<?= $fetch_review['id']; ?>">
                        </form>

                        <script>
                            document.getElementById('delete-btn-<?= $fetch_review['id']; ?>').onclick = function() {
                                swal({
                                    title: "Are you sure?",
